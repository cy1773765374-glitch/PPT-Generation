...
- 自动纠正旧配置中的 api.minimax.com 为官方 api.minimax.io 图像端点。
...
def normalize_minimax_image_api_url(raw_url: str) -> str:
    """
    MiniMax 官方图像接口域名是 api.minimax.io。
    v1.2 曾经在部分环境里被旧 .env / shell 变量覆盖为 api.minimax.com，
    该域名会触发 DNS 失败；v1.3 在运行时直接纠正，避免继续踩坑。
    """
    url = (raw_url or "").strip()
    if not url:
        url = "https://api.minimax.io/v1/image_generation"

    # 常见错误：api.minimax.com 不是当前官方 image_generation 域名。
    url = url.replace("https://api.minimax.com", "https://api.minimax.io")
    url = url.replace("http://api.minimax.com", "https://api.minimax.io")

    # 允许用户只填 base URL。
    if url.rstrip("/").endswith("api.minimax.io"):
        url = url.rstrip("/") + "/v1/image_generation"

    # 允许从 MiniMax anthropic 兼容地址推导图像端点。
    if "api.minimaxi.com/anthropic" in url or "api.minimax.io/anthropic" in url:
        url = "https://api.minimax.io/v1/image_generation"
...
def assert_hostname_resolves(api_url: str, timeout: int = 8) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "请确认服务器 DNS/代理可访问 https://api.minimax.io/v1/image_generation。"
        ) from exc
    finally:
...
class MiniMaxImageClient:
    def __init__(self) -> None:
        ...
        self.api_url = normalize_minimax_image_api_url(configured_url)
        ...
        self.network_precheck = env_bool("MINIMAX_NETWORK_PRECHECK", True)
    ...
    def generate_to_file(self, prompt: str, out_path: Path, aspect_ratio: str = "16:9") -> None:
        ...
        if self.network_precheck:
            assert_hostname_resolves(self.api_url)
        ...
        try:
            ...
            raise RuntimeError(f"MiniMax 生图请求失败：url={self.api_url}；{exc}") from exc
...
def create_cover(prs: Presentation, spec: CatalogSpec, run_name: str, assets: Dict[str, ImageAsset], now: datetime) -> None:
    ...
    add_card(slide, Inches(0.82), Inches(4.15), Inches(5.25), Inches(1.35), "目录册信息", [f"页数：{spec.page_count} 页", f"生成时间：{now.strftime('%Y-%m-%d %H:%M')}（Asia/Shanghai）", "版本：OpenClaw PPT Generation v1.3"], (37, 99, 235), 9.5)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate product catalog PPT v1.3")
    ...
    if args.json:
        ...
        if result.ok:
            # v1.3 非 JSON 输出只打印本地映射盘路径，便于飞书直接转发。
