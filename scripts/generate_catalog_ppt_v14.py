...
- 默认使用用户指定的 api.minimax.com 图像端点；不再自动改写为 .io。
...
def normalize_minimax_image_api_url(raw_url: str) -> str:
    ...
    v1.4 按用户实际环境默认使用 api.minimax.com，不再把 .com 自动改写为 .io。
    支持三种写法：
    ...
    # 允许用户只填 base URL。
    if url.rstrip("/").endswith("api.minimax.com"):
        url = url.rstrip("/") + "/v1/image_generation"

    # 如果误填了 Anthropic 兼容地址，推导到同域名图像接口。
    if "api.minimax.com/anthropic" in url:
        url = "https://api.minimax.com/v1/image_generation"
    if "api.minimaxi.com/anthropic" in url:
        url = "https://api.minimax.com/v1/image_generation"
...
def create_cover(prs: Presentation, spec: CatalogSpec, run_name: str, assets: Dict[str, ImageAsset], now: datetime) -> None:
    ...
    add_card(slide, Inches(0.82), Inches(4.15), Inches(5.25), Inches(1.35), "目录册信息", [f"页数：{spec.page_count} 页", f"生成时间：{now.strftime('%Y-%m-%d %H:%M')}（Asia/Shanghai）", "版本：OpenClaw PPT Generation v1.4"], (37, 99, 235), 9.5)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate product catalog PPT v1.4")
    ...
    if args.json:
        ...
        if result.ok:
            # v1.4 非 JSON 输出只打印本地映射盘路径，便于飞书直接转发。
