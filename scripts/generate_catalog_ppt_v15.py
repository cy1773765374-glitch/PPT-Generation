...
- 默认先使用用户指定的 api.minimax.com；如 DNS/请求失败，自动尝试国内站 api.minimaxi.com 和国际站 api.minimax.io。
...
def normalize_minimax_image_api_url(raw_url: str) -> str:
    ...
    v1.5 规则：尊重用户配置的 api.minimax.com，但不再把它当成唯一端点。
    如果用户只填 host/base URL，会自动补成 /v1/image_generation。
    如果用户误填 /anthropic 或 /anthropic/v1，会自动保留同一 host 并改成 /v1/image_generation。
    """
    url = (raw_url or "").strip()
    if not url:
        url = "https://api.minimax.com/v1/image_generation"
    ...
    path = parsed.path or ""
    if not host:
        # 兜底处理极端输入，例如 api.minimax.com/v1/image_generation 被 urlparse 解析异常。
    ...
    if "anthropic" in path or path in {"", "/"} or path.rstrip("/") == "/v1":
        return urllib.parse.urlunparse((scheme, host, "/v1/image_generation", "", "", ""))

    # 其他 base path 也强制落到同 host 的图像接口，避免错误拼接。
    return urllib.parse.urlunparse((scheme, host, "/v1/image_generation", "", "", ""))


def get_minimax_image_api_candidates() -> List[str]:
    """返回 MiniMax 图像接口候选列表，按顺序尝试。"""
    configured_url = (
    ...
    if raw_candidates:
        ...
        candidates = [
            configured_url,
            # 国内开放平台官方接口，用户常把 minimaxi.com 误写成 minimax.com。
...
def assert_hostname_resolves(api_url: str, timeout: int = 8) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "请确认服务器 DNS/代理可访问当前 MINIMAX_IMAGE_API_URL；如果你通过代理访问，建议设置 MINIMAX_NETWORK_PRECHECK=0。"
        ) from exc
    finally:
...
class MiniMaxImageClient:
    def __init__(self) -> None:
        ...
        self.api_url = self.api_urls[0] if self.api_urls else "https://api.minimax.com/v1/image_generation"
    ...
    def generate_to_file(self, prompt: str, out_path: Path, aspect_ratio: str = "16:9") -> None:
        ...
        raise RuntimeError(
            "MiniMax 生图全部候选接口均失败。"
            "请优先确认服务器 DNS/代理是否能访问 api.minimax.com 或 api.minimaxi.com；"
            "如需指定代理，可设置 MINIMAX_PROXY_URL。"
            f"详细错误：{joined}"
        )
...
def create_cover(prs: Presentation, spec: CatalogSpec, run_name: str, assets: Dict[str, ImageAsset], now: datetime) -> None:
    ...
    add_card(slide, Inches(0.82), Inches(4.15), Inches(5.25), Inches(1.35), "目录册信息", [f"页数：{spec.page_count} 页", f"生成时间：{now.strftime('%Y-%m-%d %H:%M')}（Asia/Shanghai）", "版本：OpenClaw PPT Generation v1.5"], (37, 99, 235), 9.5)
...
def generate(prompt: str, sender_name: str = "", sender_open_id: str = "", root: Optional[str] = None, image_mode: Optional[str] = None) -> GenerationResult:
    ...
    try:
        ...
        elapsed = round(time.time() - start, 1)
        windows_path = windows_path_for(pptx_path, root_path)
        # 生图失败时不生成伪 PPT，但保留错误目录，方便在 /data/share/yaq/ppt/catalog 下定位原因。
        try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate product catalog PPT v1.5")
    ...
    if args.json:
        ...
        if result.ok:
            # v1.5 非 JSON 输出只打印本地映射盘路径，便于飞书直接转发。
